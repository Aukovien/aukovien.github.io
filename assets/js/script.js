...
document.getElementById('the-alias').addEventListener('click', () => {
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeSecret(); closeMobileNav(); return; }
    konamiIdx = e.key === konamiSeq[konamiIdx] ? konamiIdx + 1 : (e.key === konamiSeq[0] ? 1 : 0);
    if (konamiIdx === konamiSeq.length) {
        document.getElementById('secret-screen').classList.add('showing');
        konamiIdx = 0;
    }
});

function closeSecret() {
    document.getElementById('secret-screen').classList.remove('showing');
}

document.getElementById('secret-screen').addEventListener('click', e => {
...
const numWatcher = new IntersectionObserver(entries => {
...
document.querySelectorAll('[data-target]').forEach(el => numWatcher.observe(el));
